class Chart {
    ...
    #updateDataBounds(offset, scale) {
        const { dataBounds, drfaultDataBounds: def } = this;
    ...
    #getMouse = (event, dataSpace = false) => {
        ...
        if (dataSpace) {
            const dataLoc = math.remapPoint(this.pixelBounds, this.drfaultDataBounds, pixelLoc);
            return dataLoc;
        }
        return pixelLoc;
    }
    ...
    #drawSamples(samples) {
        ...
        for (const sample of samples) {
            ...
            switch (this.icon) {
                case "text":
                    graphics.drawText(ctx, { text: this.styles[label].text, loc: pixelLoc, size: 25 });
                    break;
                case "image":
